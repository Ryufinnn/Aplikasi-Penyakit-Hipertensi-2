...
echo    "<td width='65%'>$row[namagjl]</td>";
...
$no++;
		...
		if(($rule11 > 0)&&($rule12 > 0)&&($rule13 > 0)){
			...
			$hasil1 = $min*0.6;
			echo "<br><h3>Kemungkinan Penyakit yang di derita  : Osteoathritis 0.6 </h3>";
			...
			$min = min($nilai1,$nilai2,$nilai3);
			$hasil2 = $min*0.45;
			echo "<br><h3>Kemungkinan Penyakit yang di derita  : Osteoathritis 0.45 </h3>";
			...
			$min = min($nilai1,$nilai2,$nilai3);
			$hasil3 = $min*0.8;
			echo "<br><h3>Kemungkinan Penyakit yang di derita  : Arthritis Rheumatoid 0.8 </h3>";
			...
			$hasil4 = $min*0.65;
			echo "<br><h3>Kemungkinan Penyakit yang di derita  : Arthritis Rheumatoid 0.65 </h3>";
			...
			$min = min($nilai1,$nilai2,$nilai3);
			$hasil5 = $min*0.70;
			echo "<br><h3>Kemungkinan Penyakit yang di derita  : Arthritis Rheumatoid 0.70 </h3>";
			...
			$min = min($nilai1,$nilai2,$nilai3);
			$hasil6 = $min*0.80;
			echo "<br><h3>Kemungkinan Penyakit yang di derita  : Arthritis Rheumatoid 0.80</h3>";
			...
			$min = min($nilai1,$nilai2,$nilai3,$nilai4);
			$hasil7 = $min*0.80;
			echo "<br><h3>Kemungkinan Penyakit yang di derita  : Arthritis Pirai 0.80</h3>";
			...
			$min = min($nilai1,$nilai2,$nilai3);
			$hasil8 = $min*0.60;
			echo "<br><h3>Kemungkinan Penyakit yang di derita  : Arthritis Pirai 0.60 </h3>";
		...
		if($akhir==0){
			echo "<h1 align='center'>Kesimpulan </h1><h4 align='center'>Anda Tidak Terdiagnosa Penyakit Radang Sendi</h4>";
		}else{
				if($akhir == $oa){
					$num_oa = number_format($oa, 2, '.', '');
					$peroa = $num_oa*100;
					echo "<h1 align='center'>Kesimpulan </h1><h4 align='center'>Anda penyakit Osteoathritis dengan kemungkinan $peroa%</h4>";
				} else if($akhir == $ar){
					$num_ar = number_format($ar, 2, '.', '');
					$perar = $num_ar*100;
					echo "<h1 align='center'>Kesimpulan </h1><h4 align='center'>Anda memderita penyakit Athritis Rheumatoid dengan kemungkinan $perar%</h4>";
				} else if($akhir == $ap){
					$num_ap = number_format($ap, 2, '.', '');
					$perap = $num_ap*100;
					echo "<h1 align='center'>Kesimpulan </h1><h4 align='center'>Anda memderita penyakit Athritis Pirai dengan kemungkinan $perap%</h4>";
				}
		}
	?>
    
    
<p><a href="cetak2.php?username=<? echo "".$_SESSION['username'].""; ?>" target="_new">Cetak Hasil Konsultasi</a></p>